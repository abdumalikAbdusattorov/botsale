...
class Index extends Component {
  ...
  render() {
    ...
    const {allOutputByPageable}=output
    ...
    return (
      <div>
        ...
        <Modal size="lg" style={{maxWidth: '1600px', width: '80%'}} isOpen={this.state.showOutputSaveOrEditModal}
          ...
          <ModalHeader>Chiqim qilish</ModalHeader>
               ...
               {this.state.reqOutput ?
                <Row>
                  <Col>
                    <Row>
                      <Col>
                        <AvField type="select" className="mt-2" placeholder="Omborxona tanlang"
                                 name="incomerWarehouseId"
                                 onChange={getIncomerWarehouse}>
                          <option>Omborxona tanlang</option>
                          {allWarehouseListWithoutOutputer ? allWarehouseListWithoutOutputer.map(item =>
                            <option value={item.id}>{item.name + " | " + item.regionName}</option>
                          ) : ''}
                        </AvField>
                      </Col>
                      <Col></Col>
