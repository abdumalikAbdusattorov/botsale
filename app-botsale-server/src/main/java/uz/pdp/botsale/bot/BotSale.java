    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage()) {
            if (update.getMessage().hasContact()) {
                ...
                else if (update.getMessage().getText().contains("Savatcha") || update.getMessage().getText().contains("Карзинка")) {
                    telegramBotService.getFutureDeletingMessageData(update.getMessage().getMessageId(),update.getMessage().getChatId());
                    telegramBotService.sendToClient(telegramBotService.showbasket(update));
                }
                else {
                    telegramBotService.getFutureDeletingMessageData(update.getMessage().getMessageId(),update.getMessage().getChatId());
